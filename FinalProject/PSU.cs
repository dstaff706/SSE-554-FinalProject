...
namespace FinalProject
{
    public class PSU : ComputerPart, IComparable
    {
        ...
        // Generates the PSU's hyperlink in the PSU Database hosted on TechPowerUp.com 
        public override void SetDatabaseLink(string dbCode)
        {
            // Replace the spaces in the GPU model with '-' and force it to be lowercase 
            string psuModel = this.Model.ToLower().Replace(" ", "-");

            // Generate the TechPowerUp link with the provided code
            string link = $"https://www.techpowerup.com/cpu-specs/{psuModel}.{dbCode}";
